...
impl ExperimentDriver for ServerSimulator {
    ...
    fn setup(&mut self, pipelines: usize, lanes: usize, messages: usize) {
        ...
        self.executor.spawn_ok(async move {
            loop {
                // fixme: (futures) This is likely wrong. Need to await on Receiver concentrator_r.
        ...
        for _ in 1 ..= lanes {
            ...
            for id in 1 ..= pipelines {
                ...
                self.executor.spawn_ok(async move {
                    loop {
                        // fixme: (futures) This is likely wrong. Need to await on Receiver r.
                        match r.try_next() {
                            Ok(Some(cmd)) => forwarder.receive(cmd).await,
                            Ok(None) => break,
                            Err(mpsc::TryRecvError { .. }) => break,
                ...
                match prev {
                    ...
                    None => self.heads.push(TestMessageSender::FuturesSender(s.clone())),
                }
            ...
            futures::executor::block_on(async move {
                ...
                match notifier {
                    TestMessageReceiver::FuturesReceiver(ref mut receiver) => {
                        // fixme: (futures) This is likely wrong. Need to await on Receiver receiver.
