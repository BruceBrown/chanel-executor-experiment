...
/// SyncForward implements a sync forwarder which can forward received message and notify
/// a Receiver when it has received a configured number of messages. There is a similar
/// ASyncForwarder which mirrors this, as async.
...
impl SyncForwarder {
    ...
    pub fn handle_action(&mut self, cmd: TestMessage) {
        match cmd {
            TestMessage::TestData(_) => {
                for sender in &self.senders {
                    for _ in 0 .. self.forwarding_multiplier {
                        send_cmd(sender, TestMessage::TestData(self.send_count));
                        self.send_count += 1;
                    }
                }
            },
    ...
    pub fn handle_notification(&mut self) {
        if self.received_count == self.notify_count {
            ...
            if let Some(notifier) = self.notify_sender.as_ref() {
                send_cmd(notifier, TestMessage::TestData(count));
            }
        }
    }
